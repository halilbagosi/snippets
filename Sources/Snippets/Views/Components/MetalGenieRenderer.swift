...
final class MetalGenieRenderer: NSObject, MTKViewDelegate {
    ...
    private static let logger = Logger(subsystem: "Snippets", category: "MetalGenieRenderer")
    ...
    private func makeShaderLibrary() throws -> MTLLibrary {
        ...
        #if SWIFT_PACKAGE
        guard let shaderURL = Bundle.module.url(forResource: "GenieShaders", withExtension: "metal") else {
            throw RendererError.missingLibrary
        }
        #else
        guard let shaderURL = Bundle.main.url(forResource: "GenieShaders", withExtension: "metal") else {
            throw RendererError.missingLibrary
        }
        #endif
