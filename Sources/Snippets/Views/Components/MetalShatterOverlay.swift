...
struct MetalShatterOverlay: View {
    ...
    var body: some View {
        Group {
            #if os(macOS)
            MetalShatterRepresentable(progress: progress, accent: accent, snapshot: snapshot)
...
#if os(macOS)
fileprivate struct MetalShatterRepresentable: NSViewRepresentable {
    var progress: CGFloat
    var accent: Color
    var snapshot: NSImage?
    ...
    func makeNSView(context: Context) -> MTKView {
        ...
        if context.coordinator.renderer == nil {
            context.coordinator.renderer = MetalShatterRenderer(mtkView: view)
        }
    ...
    class Coordinator {
        var renderer: MetalShatterRenderer?
    }
}
#endif
